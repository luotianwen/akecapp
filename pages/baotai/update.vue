			...
			handleSearch() {
			 ...
			 get(id){
				 console.log(id);
					...
					var _this = this;
					uni.request({
						url: service.getReportDStandbook(),
						data: data,
						success: (data) => { 
					console.log("data.data.data="+JSON.stringify(data.data.data));
							if (data.statusCode == 200 && data.data.code == 0) {
								...
								_this.sexs.forEach((item,index)=>{
								 ...
								 if(d.reportDStandbookImageDetailList){
									 d.reportDStandbookImageDetailList.forEach((item,index)=>{
									  _this.phos.push(item.reportImgUrl);
									  });
								 }
								if(d.reportDStandbookProductDetailList)
								d.reportDStandbookProductDetailList.forEach((item,index)=>{
									...
									if(d.reportDStandbookGradeDetailList)
								 _this._grades.forEach((item,index)=>{
									 
									 d.reportDStandbookGradeDetailList.forEach((item1,index2)=>{
					...
					success: (data) => { 
						...
						if (data.statusCode == 200 && data.data.code == 0) {
						         uni.request({
						         	...
						         	success: (data2) => {
						         		uni.hideLoading();
						         		if (data2.statusCode == 200 && data2.data.code == 0) {
						         			uni.showModal({
						         			    title: '操作成功',
						         			    content: '操作成功',
						         				showCancel:false,
