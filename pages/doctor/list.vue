<template>
	<view  >
		<uni-nav-bar leftIcon="back" @clickLeft="back()"  backgroundColor="#0faeff"  color="white" @clickRight="right" statusBar="false" right-text="新增" title="医生信息"></uni-nav-bar>
		...
		<uni-list  >
			...
			<uni-list-item :title="tab.hospitalName+'        '+tab.doctorName"   @click="click(tab)" :rightText="tab.hospitalId+'          '+tab.doctorName"   :show-extra-icon="true"   />
			 </block>
		</uni-list>
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 click(tab){
					 uni.navigateTo({
						url:"add?d="+encodeURIComponent(JSON.stringify(tab))
					 })
			 },
			  right(){
			 	 uni.navigateTo({
